...
function GifReader(buf) {
  ...
  this.decodeAndBlitFrameRGBA = function (frame_num, pixels) {
    var frame = this.frameInfo(frame_num); // 计算出总像素点数量

    var num_pixels = frame.width * frame.height; // 创建像素点值

    var index_stream = new Uint8Array(num_pixels); // At most 8-bit indices.

    GifReaderLZWOutputIndexStream(buf, frame.data_offset, index_stream, num_pixels); // console.log(index_stream)

    var palette_offset = frame.palette_offset; // 计算出空像素点值
...
function GifReaderLZWOutputIndexStream(code_stream, p, output, output_length) {
  ...
  while (true) {
    ...
    while (cur_shift < 16) {
      ...
      if (subblock_size === 1) {
        // Never var it get to 0 to hold logic above.
...
function () {
  ...
  _createClass(GIF, [{
    ...
    value: function init(overlying) {
      ...
      _ts.__status = {
        status: 'init',
        // 状态，默认初始化（init、playing、played、pause、stop）
        frame: 0,
        // 当前帧数
        loops: 0,
        // 连续循环播放次数，停止播放会清0
    ...
    key: "play",
    value: function play(loop, callback) {
      var _this = this;

      // 没有纹理材质时抛出错误
      if (!this.textures.length) throw new Error('没有可用的textures'); // 纹理材质只有一帧时不往下执行

      if (this.textures.length === 1) return;
      var status = this.__status,
          attr = this.__attr,
          time = 0; // 当状态是停止的时候，将播放次数清0
    ...
    key: "runEvent",
    value: function runEvent(type, status) {
      var temp = this.temp;
    ...
    key: "getExeName",
    value: function getExeName(filePath) {
    ...
    key: "createSprite",
    value: function createSprite(esource, resources) {
      var _ts = this;

      var Sprite = PIXI.Sprite,
          imgSrc = esource,
          exeName = this.getExeName(imgSrc.toLocaleLowerCase()); // 文件扩展名为gif或png则返回对应的名称，其它反返回other
      ...
      var funs = {
        'gif': function gif() {
          ...
          _ts.framesDelay = gifDecodeData.delayTimes; // _ts.play();
          // 返回精灵并将纹理材质设置为第一帧图像
    ...
    key: "apngResourceToTextures",
    value: function apngResourceToTextures(resource) {
      var _ts = this;
    ...
    key: "gifResourceToTextures",
    value: function gifResourceToTextures(resource) {
      ...
      for (var i = 0; i < gifFramesLen; i++) {
        ...
        ctx = canvas.getContext('2d'); //创建一块空白的ImageData对象

        if (!this.__attr.overlying) {
          imageData = ctx.createImageData(gifWidth, gifHeight); // 将第一帧转换为RGBA值，将赋予到图像区

          gif.decodeAndBlitFrameRGBA(i, imageData.data);
        } else {
          if (last == null) last = ctx.createImageData(gifWidth, gifHeight);
          imageData = last; // 将第一帧转换为RGBA值，将赋予到图像区
        ...
        ctx.putImageData(imageData, 0, 0);
        spriteSheet = new PIXI.BaseTexture.fromCanvas(canvas); // console.log(imageData.data)

        obj.textures.push(new PIXI.Texture(spriteSheet, new PIXI.Rectangle(0, 0, gifWidth, gifHeight)));
      }

      ;
      console.timeEnd('testForEach'); // document.body.appendChild(canvas);
